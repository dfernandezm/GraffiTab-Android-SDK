...
class GTJsonCall<T> extends GTCall<T> {
    ...
    public GTJsonCall(Call<Map<String,T>> wrappedCall, String propertyNameToExtract, Gson gson,
    ...
    @Override
    void handleSuccessfulResponse(GTResponseHandler<T> responseHandler) {
        GTResponse<T> gtResponse = new GTResponse<T>();
        T decodedResponse = (T) resolvedResponse.body().get(propertyNameToExtract);
        gtResponse.setObject(decodedResponse);

        gtResponse.setResultCode(ResultCode.OK);
        gtResponse.setApiEndpointUrl(super.apiEndpointUrl);
